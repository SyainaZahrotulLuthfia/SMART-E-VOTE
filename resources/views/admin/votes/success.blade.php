        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header pb-0 card-no-border">
                    <h4>Sudah Menggunakan Hak Suara</h4>
                    <span>Total <strong>{{ count($boxes) }}</strong> siswa sudah memilih untuk voting:
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive theme-scrollbar">
                        <table id="laporanTable" class="table table-striped table-bordered mb-0" style="width:100%">
                            <thead class="table-primary text-center">
                                <tr>
                                    <th style="min-width: 50px">No</th>
                                    <th style="min-width: 150px">Nama</th>
                                    <th style="min-width: 100px">Kelas</th>
                                    <th style="min-width: 180px">Waktu Memilih</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($boxes as $box)
                                    <tr>
                                        <td class="text-center"></td> <!-- Nomor akan diisi lewat JS -->
                                        <td>{{ $box->user->name ?? '-' }}</td>
                                        <td>{{ $box->user->classroom->classroom ?? '-' }}</td>
                                        <td>{{ $box->created_at->translatedFormat('d F Y H:i') }} WIB</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-4">Belum ada siswa yang
                                            memilih.</td>
                                    </tr>
                                @endforelse
                            </tbody>
